...
public class Validation<T> {
    ...
    public Validation<T> validate(Predicate<? super T> validation, String message) {
        try {
    ...
    public Validation<T> validate(ValidationDefinition<T> def) {
        return validate(def.getValidation(), def.getMessage());
    }

    public <U> Validation<T> validate(
